    ...
    <table class="table_lists table_click">
        ...
        <tbody>
            ...
            <tr id="<?php echo "formli1".$k?>">
                <td><input type="checkbox" class="chk_list" value="<?php echo $v["user_id"];?>"></td>
                ...
                <td><?php echo $v["uqq"];?></td>
                <td><?php echo $this->vars->get_field_str("user_status",$v['ustate'],'');?></td>
                <td>
                    <?php $thisc->echoButton("{$thisc->level}","/back/{$thisc->controllerId}/edit?id={$v["user_id"]}","编辑");?>
                    <?php 
                    $s = $v['ustate'] == 0 ? 1 : 0;
                    $thisc->echoButton("{$thisc->level}03","javascript:status({$v['user_id']},{$s});",$this->vars->get_field_str("user_status_action",$v['ustate'],''));?>
...
<div class="footer_fixed">
    <div class="box_1000">
        <span>操作：</span>
        <?php $thisc->echoButton("{$thisc->level}01","/back/{$thisc->controllerId}/add","添加用户",'btn3');?>
        <?php $thisc->echoButton("{$thisc->level}02","javascript:del_data();","批量删除",'btn3');?>
        <?php $thisc->echoButton($thisc->level,"/back/{$thisc->controllerId}","用户列表",'btn3');?>
