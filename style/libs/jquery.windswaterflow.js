...
*        endSelector:'#end',
*        boxTemplate: '<li class="box hide"><a class="img" href="{url}"><img src="{img}" alt="" /></a><span class="title">{title}</span></li>',
*        columnWidth: 240,
*        marginWidth: 14,
*        marginHeight: 16,
*        ajaxServer: 'data.php',
...
; (function($) {
    $.fn.windswaterflow = function(options) {
        ...
        var defaults = {
            itemSelector: '', //初始页面提示div
            loadSelector: '', //容器名称
            endSelector:'', //结束提示标签名称
            boxTemplate: '', //容器模板
            columnWidth: 240,
            marginWidth: 14,
            marginHeight: 16,
            ...
            pageNumber: 0, //请求页面数量
            init: false,
            initBoxNumber: 20, //初始容器数量
            scroll: false,
            scrollBoxNumber: 10 //请求数量
        };
        //参数合并
            ...
            $(window).scroll(function() {
                ...
                if (scrollt + clienth >= scrollh && _SolStatus == 0 && _init == 1) {
                    if (param.maxPage == 0 || param.pageNumber <= param.maxPage) {
                        ...
                        postPara[param.pageParam] = parseInt(param.pageNumber);
                        //判断结束
                        if(parseInt(param.pageNumber) >= parseInt(param.maxPage)){
                            $(param.endSelector).show();
                        }
                        setTimeout(function() {
                            $.getJSON(param.ajaxServer, postPara, function(data) {
                                appendBox(data, 'add');
